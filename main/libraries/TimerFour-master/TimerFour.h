...
#define F_TIMER (F_PLL/2)
  ...
  public:
	...
	setPwmDuty(pin, duty);
	if (pin == TIMER4_A_PIN) {
		*portConfigRegister(TIMER4_A_PIN) = PORT_PCR_MUX(3) | PORT_PCR_DSE | PORT_PCR_SRE;
	} else if (pin == TIMER4_B_PIN) {
		*portConfigRegister(TIMER4_B_PIN) = PORT_PCR_MUX(3) | PORT_PCR_DSE | PORT_PCR_SRE;
	}
    }
    void pwm(char pin, unsigned int duty, unsigned long microseconds) __attribute__((always_inline)) {
	if (microseconds > 0) setPeriod(microseconds);
    ...
    void disablePwm(char pin) __attribute__((always_inline)) {
	if (pin == TIMER4_A_PIN) {
		*portConfigRegister(TIMER4_A_PIN) = 0;
	} else if (pin == TIMER4_B_PIN) {
		*portConfigRegister(TIMER4_B_PIN) = 0;
	}
    }
